...
const PaymentScreen = ({ navigation }) => {
    ...
    const obtenerDataUsuario = async () => {    
        try { 
            ...
            if (storedToken !== null) {
                ...
                const response = await mainApi.get('/obtener-valor-deuda', {
                    headers: {
                        ...(storedToken && { Authorization: `Bearer ${storedToken}` })
                    }
                })
                const userDataFromAPI = response.data;
                setValueDebData(userDataFromAPI);
            }
        } catch (error) {
    ...
    const handlePay = async  (userDeb) => {
      ...
      if (selectedCard) {
        ...
        try { 
            const storedToken = await AsyncStorage.getItem('access_token');
            if (storedToken) {
                const saldoData = {soci_fact_saldo: userDeb?.soci_fact_valor};
                const response = await mainApi.post('/pagar-deuda', saldoData, {
                    headers: {
                        ...(storedToken && { Authorization: `Bearer ${storedToken}` })
                    }
                })
                setIsLoading(false);
                Alert.alert('Pago Exitoso', 'Tu pago se ha realizado correctamente', [
                    {
                        text: 'OK',
                        onPress: () => {
    ...
    const handleRefresh = () => {
        // Realiza la lógica de actualización aquí (por ejemplo, llama a tus funciones obtenerDataUsuario)
        obtenerDataUsuario();
      };
                ...
                >
                    <View
                        ...
                        <View style={{ flex: 1, padding: 16 }}>
                            ...
                            <Text style={{fontSize: 26, color:'#0134bd', fontWeight: 'bold' }}>${ userDeb?.soci_fact_valor }</Text>
                            </View>                       
